...
from workers.common.backtest.backtest_candle import BacktestCandle
from workers.common.command import ApiCommand
from workers.common.db import DBEnum, Database
...
class BinanceBacktestWorker(RpcBase):
    def __init__(self):
        super().__init__()
        ...
        for target in kwargs['targets']:
            ...
            if target_name in self._symbols:
                self._backtestCandle[target_name] = BacktestCandle(
                    self._db, DBEnum.BINANCE_QUOTE_DB, self._conn,
...
async def main() -> None:
    ...
    backtester = BinanceBacktestWorker()
    await backtester.preload()
    await conn.connect()
    await conn.run_with_bus_queue(enums.WorkerType.Backtest, backtester, True)
